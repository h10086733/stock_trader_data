...
def run_one(arr: dict[str, np.ndarray], params: tuple[float, ...]) -> dict[str, float]:
    csi_short_threshold, hs300_short_filter, short_volume_multiplier, high_drawdown = params
    opens = arr["open"]
    closes = arr["close"]
    csi = arr["csi"]
    hs300 = arr["hs300"]
    ...
    for i in range(len(opens) - 1):
        if pending != position:
            open_price = opens[i]
            if position == -1:
                ret = entry_price / open_price - 1.0
        ...
        short_context = bool(
            valid[i]
            and csi[i] < -csi_short_threshold
            and hs300[i] < hs300_short_filter
            and vol[i] < short_volume_multiplier
        )
        if wave_active and not short_context:
            wave_active = False
            wave_blocked = False
        ...
        if short_context:
            if not wave_active:
                wave_active = True
                wave_blocked = dd10[i] > high_drawdown
            pending = 0 if wave_blocked else -1
        else:
            pending = 0

    if position == -1:
        ret = entry_price / closes[-1] - 1.0
        trade_returns.append((ret - fee) * 100.0)
...
def main() -> int:
    ...
    grid_path = OUT_DIR / "csi1000_short_only_stability_grid.csv"
    results.to_csv(grid_path, index=False)

    summaries = []
    for col in ("csi_short_threshold", "hs300_short_filter", "short_volume_multiplier", "high_drawdown"):
        summary = (
            results.groupby(col)
            .agg(
    ...
    summary_path = OUT_DIR / "csi1000_short_only_stability_summary.csv"
    ...
    print("\nTop by return:")
    print(results.sort_values(["total_return_pct", "return_to_dd"], ascending=False).head(20).to_string(index=False))
    print("\nTop by return_to_dd:")
