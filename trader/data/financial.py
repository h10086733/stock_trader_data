...
实现迁移到 trader.data.financial。HTTP session 复用 core；findata 专用的
抓取调速参数（REQUEST_INTERVAL=0.35 / MAX_RETRIES=3）和私有 safe_get
异常口径保持不变。pb_history / roe_history 为本模块私有表。

使用方式：
  python -m trader.data.financial --stocks "000630,002092"
"""
import sqlite3
import requests
import time
...
from trader.core import config, logging as _tlog
from trader.core.http import make_session as _make_core_session

DB_PATH          = str(config.DB_PATH)
REQUEST_INTERVAL = 0.35
RETRY_INTERVAL   = 5.0
MAX_RETRIES      = 3
FINDATA_URL      = config.FINDATA_URL
...
def make_session():
    """findata 专用 session：复用 core 的 Session（含连接池+重试），叠加财务接口默认头。"""
    s = _make_core_session()
...
def safe_get(url, params, timeout=15):
    """findata 私有 safe_get：异常口径较宽（except Exception），重试 MAX_RETRIES 次。"""
