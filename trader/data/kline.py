"""
股票数据入库系统 — 阶段一：股票列表 + 历史价格（分层重构版）

实现已迁移到 trader.data.kline。基础设施（HTTP session / safe_get / 日志 /
DB 连接 / 代码归一化）改用 trader.core，消除与 findata.py 的重复。
业务逻辑（fetch_stock_list / fetch_kline / sync / init_db / main）保持不变。

使用方式：
  python -m trader.data.kline --sync   # 或经 fetch.py shim
"""
...
from trader.core import config, http as _http, logging as _tlog
from trader.core.codes import to_sina_symbol

# ── 配置（取自 core.config，保留 fetch 专用常量）──────────────────────
DB_PATH          = str(config.DB_PATH)
LOG_FILE         = str(config.LOG_FILE)
REQUEST_INTERVAL = config.REQUEST_INTERVAL
RETRY_INTERVAL   = config.RETRY_INTERVAL
MAX_RETRIES      = config.MAX_RETRIES
FAIL_ALERT_RATIO = 0.05
HISTORY_START    = config.HISTORY_START
SINA_BATCH_SIZE  = 100
SINA_FALLBACK_BATCH_SIZE = 20
MARKET_CAP_BATCH_SIZE = 80

STOCK_LIST_URL = config.STOCK_LIST_URL
KLINE_URL      = config.KLINE_URL
SINA_PRICE_URL = config.SINA_PRICE_URL
SINA_REFERER   = config.SINA_REFERER
EASTMONEY_QUOTE_URLS = config.EASTMONEY_QUOTE_URLS

USER_AGENTS = _http.USER_AGENTS

# ── HTTP / 日志：复用 core 实现，绑定为模块级名字供下方业务逻辑调用 ──
get_headers = _http.eastmoney_headers
get_sina_headers = _http.sina_headers
make_session = _http.make_session
safe_get = _http.safe_get
safe_get_sina = _http.safe_get_sina
