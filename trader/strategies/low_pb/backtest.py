"""
低PB+ROE亏损策略回测 v2（分层重构版）
=======================================
实现迁移自 backtest2.py。基础设施（DB 路径 / 日志）改用 trader.core，
业务逻辑（抓取 / 建表 / 回测 / 报告）保持不变。v2_stocks / v2_signals /
v2_trades 为本模块私有表。原 backtest2.py 保留为向后兼容 shim。
